...
if [ $(dpkg-query -W -f='${Status}' apache2 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
    ...
    sudo a2ensite pma.drupal7-study.loc.conf > /dev/null 2>&1
...
if [ $(dpkg-query -W -f='${Status}' nginx 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
    ...
    sudo ln -s /etc/nginx/sites-available/pma.drupal7-study.loc.conf /etc/nginx/sites-enabled/ > /dev/null 2>&1
