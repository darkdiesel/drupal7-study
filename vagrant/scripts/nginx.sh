...
if [ -d "$MAIN_SITE_DIR" ]; then
    ...
    if [ ! -f "/vagrant/configs/etc/nginx/ssl/drupal7-study.loc_bundle.crt" ]; then
        if [ ! -f "/vagrant/configs/etc/nginx/ssl/drupal7-study.loc_private.key" ]; then
            sudo openssl req -new -newkey rsa:1024 -nodes -keyout /vagrant/configs/etc/nginx/ssl/drupal7-study.loc_private.key -x509 -days 500 -subj /C=RU/ST=Grodno/L=Grodno/O=Companyname/OU=User/CN=drupal7-study.loc/emailAddress=admin@drupal7-study.loc -out /vagrant/configs/etc/nginx/ssl/drupal7-study.loc_bundle.crt > /dev/null
        fi
    fi
fi
...
if [ -d "$MAIN_SITE_DIR" ]; then
    sudo ln -s /etc/nginx/sites-available/drupal7-study.loc.conf /etc/nginx/sites-enabled/ > /dev/null
fi
log_end_msg 0
