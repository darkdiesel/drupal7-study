...
if [ -d "$MAIN_SITE_DIR" ]; then
    sudo openssl req -new -newkey rsa:1024 -nodes -keyout /etc/nginx/ssl/drupal7-study.loc_private.key -x509 -days 500 -subj /C=RU/ST=Grodno/L=Grodno/O=Companyname/OU=User/CN=drupal7-study.loc/emailAddress=admin@drupal7-study.loc -out /etc/nginx/ssl/drupal7-study.loc_bundle.crt > /dev/null 2>&1
fi
...
if [ -d "$MAIN_SITE_DIR" ]; then
    sudo ln -s /etc/nginx/sites-available/drupal7-study.loc /etc/nginx/sites-enabled/ > /dev/null 2>&1
fi

sudo ln -s /etc/nginx/sites-available/pma.drupal7-study.loc /etc/nginx/sites-enabled/ > /dev/null 2>&1
